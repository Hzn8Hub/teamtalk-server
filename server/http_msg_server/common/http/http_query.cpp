/*
 ...
 Update Time: Thu 15 Jun 2023 00:42:51 CST
 brief:
*/
...
void http_query_timer_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam) {
  ...
  if (year != g_last_year || mon != g_last_month || mday != g_last_mday) {
    ...
    log_info("a new day begin, g_total_query=%u ", g_total_query);
    g_total_query = 0;
    g_last_year = year;
    g_last_month = mon;
    g_last_mday = mday;
...
void CHttpQuery::DispatchQuery(std::string& url, std::string& post_data, CHttpConn* pHttpConn) {
   ...
   */
  log_info("DispatchQuery, url=%s, content=%s ", url.c_str(), post_data.c_str());
  ...
  if (!reader.parse(post_data, value)) {
    log_info("json parse failed, post_data=%s ", post_data.c_str());
    pHttpConn->Close();
    return;
  }
...
void CHttpQuery::_QueryChangeMember(const std::string& strAppKey, Json::Value& post_json_obj, CHttpConn* pHttpConn) {
  ...
  try {
    ...
    uint32_t user_cnt = post_json_obj["user_id_list"].size();
    log_info(
      "QueryChangeMember, user_id: %u, group_id: %u, modify type: %u, "
      "user_cnt: %u. ",
      user_id,
      group_id,
      modify_type,
